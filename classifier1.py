...
def train(dataset, n_hidden=50, batch_size=100, epochs=100, learning_rate=0.01, model='nn', l2_ratio=1e-7,
    ...
    for epoch in range(epochs):
        ...
        for input_batch, target_batch in iterate_minibatches(train_x, train_y, batch_size):
            mid_loss = train_fn(input_batch, target_batch)
            #if math.isnan(mid_loss):
            #    mid_loss = 1e-8
            loss += mid_loss
        loss = round(loss, 3)
        print('Epoch {}, train loss {},took {:.3f}s'.format(epoch, loss,time.time() - start_time))
